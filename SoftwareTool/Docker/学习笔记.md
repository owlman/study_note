...
当然了，我们在刚刚安装完 Docker 时本地应该是没有任何镜像的，但由于之前为了测试安装是否正确，我们已经从 Docker Hub 中下载了一个名为`hello-world`测试镜像，所以读者会在上述镜像列表中看到它。在专业术语中，大家将镜像从 Docker Hub 这类远程仓库中下载到本地的操作称之为**拉取（pull）**。现在，如果读者想要下载最新版本的 Ubuntu 镜像，就需要执行以下操作将它拉取到本地：
...
如你所见，`docker image pull`命令会负责将指定的镜像从远程镜像仓库服务的仓库中拉取到本地。在不做任何自定义配置的情况下，Docker 默认的远程镜像仓库是 Docker Hub。具体到上述操作中，`docker image pull ubuntu:latest`命令的作用就是从 Docker Hub 中将标签为 latest 的 Ubuntu 镜像拉取到本地。而通过`docker image ls`命令，我们可以看到，该镜像的大小只有 72.8MB。
