...
除此之外，如果我们想将容器的某个运行状态保存下来，以便日后使用，也可以通过执行`docker commit <容器名或容器ID> <镜像名>`命令将容器重新保存为新的镜像。如果希望将这些镜像传递给别人使用，我们还通过`docker save -o <文件名> <镜像名>`命令现有的某个镜像打包成文件，然后别人在收到该文件之后，就可以通过执行`docker load -i <文件名>`命令将该镜像加载到本地。
