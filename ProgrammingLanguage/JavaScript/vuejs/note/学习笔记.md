...
到目前为止，大家所看到的步骤与在标准的 HTML 标签上注册事件处理函数并没有什么区别，接下来的任务是思考如何让我的假设成真。也就是说，现在要解决的问题是如何让`<say-hello>`组件真正实现对"show-message"事件的监听。对此，Vue.js 框架提供的解决方案是：利用组件内部的某个 HTML 标签的标准事件来触发整个组件的自定义事件。具体来说就是，我需要对`sayHello.vue`文件做出如下修改：
