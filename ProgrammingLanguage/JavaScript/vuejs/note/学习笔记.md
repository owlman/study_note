...
在掌握了如何构建与编译 Vue 组件的基础知识之后，接下来就可以试着来构建一些更具实际用处的组件了。而为了让组件更具实际用处，与组件相对应的自定义标签元素也必须要具备监听事件的能力，并且要能允许组件的用户注册事件处理函数。下面，我会通过为之前的`say-hello`组件增加一个要监听的自定义事件，并为该事件注册处理函数的过程来学习如何赋予组件事件处理能力。

首先，我会暂且假设`<say-hello>`标签所对应的组件已经在监听一个名为"show-message"的自定义事件，并在`index.htm`中使用`v-on`指令为其指定了事件处理函数，具体代码如下：
...
接下来，只需要和之前一样，在`main.js`中与上述页面对应的 Vue 实例中添加该事件处理函数的实现，像这样：
...
到目前为止，大家所看到的步骤与在标准的 HTML 标签上注册事件处理函数并没有什么区别。接下来的任务是让我做的假设成真。也就是说，现在要解决的问题是如何让`<say-hello>`标签对应的 Vue 组件响应这个自定义的"show-message"事件。对此，Vue.js 框架提供的解决方案是：利用组件内部的某个 HTML 标签的标准事件来触发整个组件的自定义事件。具体来说就是，我需要对`sayHello.vue`文件做出如下修改：
...
如你所见，我首先在组件中添加了一个显示文本为"弹出对话框"的按钮元素，然后为该按钮注册了单击事件的处理函数，最后在这个单击事件的处理函数中，我通过调用`this.$emit()`方法通知了当前组件的调用方（即`index.htm`页面）："show-message"事件被触发了。在这里，我们提供给`this.$emit()`方法的第一个实参应该是一个用于指定该组件被触发的事件名称，除此之外，如果还有要传递给该组件事件处理函数的实参，还可以在后面加上这些实参，例如这里的`this.who`。这样一来，`<say-hello>`标签的用户只需要单击该组件中的"弹出对话框"按钮，就可以触发"show-message"事件了。
