...
在这部分笔记中，笔者记录一些在学习 Node.js 环境时所编写的基本用法示例。首先，我们会在`ProgrammingLanguage/JavaScript`目录下创建一个用于学习 Node.js 的`nodejs`目录，该文件夹下将设置有两个目录，其中`note`目录用于存放笔记的`markdown`源码，而`code`目录则用于存放笔记中所记录的示例。
...
Node.js 是一个 JavaScript 运行时环境，它移植了 Google Chrome 浏览器的 V8 引擎，通常被用作 JavaScript 在浏览器之外的宿主环境。
