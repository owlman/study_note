...
4. 在`src/01_sayhello`目录下执行`vim 01-readFile.js`命令打开脚本文件，输入如下代码：
...
本示例将以生成个人信息页面为例，演示在服务器端基于 Node.js 使用`art-template`模板引擎来生成网页。为此，我们需要在`src`目录下执行`mkdir 04_useTemplating_engine`命令来创建这一示例的目录。

#### `art-template`模版引擎的基本语法

首先，我们先来示范一下`art-template`模版引擎的基本语法，请跟着以下步骤来构建示例：

1. 在`src/04_useTemplating_engine`目录下执行`npm install art-template --save`命令，安装将`art-template`安装到当前示例项目中。
...
`MongoDB`是非关系型数据库的一个典型代表，我们在这里用它来示范 Node.js 对关系型数据库的处理。在 Node.js 中操作`MongoDB`数据库有两个扩展包可以选择，下面我们一一来演示。首先，在`src/06_database`目录下执行`mkdir mongodb`命令，并执行以下步骤：
...
1. 在`src/06_database/mongodb`目录下执行`npm install mongoose`命令，将`mongoose`包安装到当前示例项目中。

2. 在`src/06_database/mongodb`目录下执行`touch useMongoose.js`，创建一个脚本文件，具体如下：
...
3. 保存所有文件后，在`src/06_database/mongodb`目录下执行`node useMongoose.js`命令，结果如下：
...
3. 在`src/07_Expressjs/views/`目录创建一个名为`hello.htm`的模版文件，内容如下：
