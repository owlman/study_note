...
由于Node.js只是`JavaScript`语言的运行时环境，所以它并不会限制`JavaScript`代码的具体运行形式。为了说明这一点，下面将使用四种形式来说“Hello”。

#### 1. 终端输出

创建第一个Node.js脚本，步骤如下：

1. 在`src`目录下执行`touch 01-sayHello.js`命令。
...
4. 保存文件后，在`src`目录下执行`node 01-sayHello.js`命令，结果如下：
...
1. 在`src`目录下执行`touch 01-readFile.js`命令。
...
4. 保存文件后，在`src`目录下执行`node 01-readFile.js`命令，结果如下：

    ![Node.js脚本读取文本文件](img/AB-7.png)

用Node.js脚本写文本文件，步骤如下：

1. 在`src`目录下执行`touch 01-writeFile.js`命令。
...
3. 保存文件后，在`src`目录下执行`node 01-writeFile.js`命令，结果如下：
...
1. 在`src`目录下执行`touch 01-webServer.js`命令。
...
3. 保存文件后，在`src`目录下执行`node 01-webServer.js`命令，并用浏览器访问`http://localhost:8080/`，结果如下：

    ![Node.js脚本创建Web服务](img/AB-9.png)

#### 4. TCP服务

1. 在`src`目录下执行`touch 01-tcpServer.js`命令。
...
3. 保存文件后，在`src`目录下执行`node 01-tcpServer.js`命令，并用`telnet localhost 7000`命令测试该服务器，结果如下：
