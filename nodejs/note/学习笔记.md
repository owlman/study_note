...
- 学习资料
  - 视频教程：
    [黑马程序员-nodejs教学课程](https://www.bilibili.com/video/av27670326?p=1)
  - 书籍教材：
...
本项目将致力于演示Node.js的自定义模块语法以及部分ES6新增的语法。首先，我们要在`src`目录下执行`mkdir 02_module`命令来创建这一项目的目录。
...
本项目将致力于用Node.js模拟一个类似于Apache的Web服务器，处理浏览器端的请求，将相关的页面响应给浏览器。首先，我们要在`src`目录下执行`mkdir 03_likeApache`命令来创建这一项目的目录。然后执行以下步骤：
...
3. 保存所有文件后，在`src/02_module`目录下执行`node 02-test.js`命令，结果如下：
