...
4. 如果想要执行这个 Hello World 程序，我们就只需要在该项目的根目录下执行`cargo run`命令即可，该目录在 Powershell 中的执行效果如下图所示。
    ...
    在这里，`cargo run`命令其实执行了两个操作：首先将源代码编译成了可执行的二进制文件，并将其保存在项目根目录下的`target`目录中。然后再执行该二进制文件，并在终端中输出“Hello World”字样的信息。如果读者只想执行源代码的编译操作，也可以改为执行`cargo build`命令，效果如下图所示。
