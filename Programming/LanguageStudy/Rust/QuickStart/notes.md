...
2010 年， Rust 项目首次面向向公众发布，同年开始改写自托管编译器（基于 LLVM 后端）来替换原来用 OCaml 写的编译器，在 2011 年实现了自我编译。

第一个 stable 版本的 Rust 1.0 到 2015 年才首次发布，随后按照“6-week train model”进行发布，可能每天都会产生一个nightly 版本，固定每 6 周发布一个 beta 版本，但仍需要经过测试才会合入 stable 版本，就像火车一样按照时刻表发车。
