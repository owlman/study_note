...
在 Rust 开发工具中，Cargo 除了是包管理器之外，同时也是项目的自动化构建工具。在这里，我们可以先使用该工具来新建一个 Hello World 项目，以便了解一下 Rust 项目的基本结构以及基本构建步骤，其具体操作如下。
...
4. 如果想要执行这个 Hello World 程序，我们就只需要在该项目的根目录下执行`cargo run`命令即可，该目录在 Powershell 中的执行效果如下图所示。

    ![Hello World](./img/hello_world.png)

    在这里，`cargo run`命令其实执行了两个操作：第一个操作是将源代码编译成了可执行的二进制文件，并将其保存在项目根目录下的`target`目录中；第二个操作是在当前终端环境中执行该二进制文件，以便输出“Hello World”字样的信息。如果读者只想执行源代码的编译操作，也可以改为执行`cargo build`命令，该命令的执行效果如下图所示。

    ![Hello World](./img/cargo_build.png)
