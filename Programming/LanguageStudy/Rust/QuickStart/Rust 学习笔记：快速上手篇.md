...
现在，读者已经初步接触了 Rust 项目的基本构建方法，下面，让我们再通过一个更复杂一些的 Hello World 示例来巩固一下目前所学习到的知识并做一些扩展。
...
下面，让我们来详细讲解一下`main.rs`文件中的源码，看看这一版的 Hello World 与之前有哪些不同，具体如下。

- 首先，我们使用了`use`关键字将 Rust 语言标准库中的`std::io`模块中定义的函数引入到了当前模块所在的命名空间中，以便可以调用`stdout()`和`stdin()`这两个函数来实现在命令行界面中的输入/输出。
- 接下来，我们用`fn`关键字定义了一个名为`say_hello()`的函数来专门执行与 Hello World 相关的操作，并在`main()`函数中调用它。这也是函数这个基本编程单元的用法，即封装一组相关的操作以便可重复调用。
