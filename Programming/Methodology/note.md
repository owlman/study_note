...
如果读者经常参与一些线上的交流，就会发现一个普遍存在的问题，即人们在讨论计算机编程议题时实际上绝大多数都在讨论编程语言，例如，初学者应该先学习 C 还是 Java，Rust 或 Go 值不值得学习等。但在笔者看来，编程语言即使再重要，也只是语言对于思想的表达意义。举例来说，如果我想写一封情书，无论选择用英文写还是中文写，能解决的都只有如何精确地、有效地传递我对爱人的爱慕之情，决定这封情书核心质量的是我对爱人此时此刻的思恋，而不是文章本身用了欧美的十四行诗还是蝶恋花之类的宋词。编程工作也是如此，最终决定我们工作质量的是语言所要表达的编程思想。所以，当我们陷于一些诸如“Java 性能很不好啦，C 才是永远的王道”之类的争论时，首先要搞清楚的是自己与争论对象想要表达的思想内容是否属于同一领域的议题，不要和人吵了三天三夜之后发现对方是做跨平台终端应用的，自己是做 JVM 的，而 Java 这门语言也从来没被拿来实现中间件，这简直就和讨论“关公战秦琼谁会赢”一样毫无意义。
...
- **面向过程编程（Procedural programming）**：，又称过程化编程，这是一种通过定义函数以及抽象数据结构的方式来实现模块化设计的编程方法。在该编程方法之下，程序的数据逻辑将通过抽象数据结构（Abstract Data Type，可简称 ADT）的方式来进行分组定义，而其接口机制的规则将通过一系列函数的定义来完成设计。面向过程面向过程编程主要流行于 20 世纪 50 年代至 70 年代，代表性的编程语言包括 Fortran、BASIC、Pascal 与 C 等。
