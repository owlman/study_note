...
- `<meta>`标记：该标记用于定义网页的具体元数据，例如我们可以用该标签将网页所使用的字符集定义为UTF-8。
...
和画家在拿到画布之后首要完成基本的构图作业一样，网页设计师们在定义好一个网页文档之后，接下来要完成的工作是网页的布局设计。在HTML5标准发布之前，网页的布局工作基本上是依靠`<div>`标记来完成的。该标记的作用是在网页中定义一个块状显示元素，这是网页设计中会用到的、最基本的布局工具，例如在下面的代码中，读者将会看到一个`id="card"`的块状元素，它的功能在相关的CSS样式作用下在网页中显示一个类似名片的卡片形态。
...
在CSS 3标准制定的规则中，一段完整的样式定义通常由选择器、属性名称和属性值三个语法单元，以及在必要时才会添加的代码注释组成，其基本语法如下：
...
在使用CSS定义外观样式的过程中，网页设计师们很大一部分的工作都与尺寸和配色问题有关，因为这两个问题涉及到如何在网页中呈现整体页面布局、图文信息以及用户交互界面等元素。下面先来介绍在网页设计工作中会涉及到的尺寸问题。
