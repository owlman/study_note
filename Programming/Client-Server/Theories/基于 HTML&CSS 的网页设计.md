...
下面，我们将通过模拟设计一个图文报告类的网页模板来演示一下上述HTML标记的使用方法，该示例被保存在本笔记所在目录下的`examples/report`目录中，我们在该目录中创建了一个名为`index.html`的文件，并在其中输入了如下HTML代码。

```html
```

然后，我们使用浏览器打开该文件，就可以看到如下效果。
```
