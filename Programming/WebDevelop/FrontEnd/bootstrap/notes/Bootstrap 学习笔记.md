...
- **链接文本样式**：对于页面中用`<a>`标记的链接文本，Bootstrap框架提供了一系列的预定义样式类，以便设计师们赋予它们一些具有特定效果的样式。下面，我们就逐一来介绍这些样式类：
  - 如果我们想设置链接文本样式中的文本颜色，可以使用`link-*`样式类来设置，其中`*`的取值及其具体的效果如下：
    - `link-primary`：该样式类会为其作用的链接文本元素添加主色系的效果；
    - `link-secondary`：该样式类会为其作用的链接文本元素添加次色系的效果；
    - `link-success`：该样式类会为其作用的链接文本元素添加成功色系的效果；
    - `link-danger`：该样式类会为其作用的链接文本元素添加危险色系的效果；
    - `link-warning`：该样式类会为其作用的链接文本元素添加警告色系的效果；
    - `link-info`：该样式类会为其作用的链接文本元素添加信息色系的效果；
    - `link-light`：该样式类会为其作用的链接文本元素添加浅色系的效果；
    - `link-dark`：该样式类会为其作用的链接文本元素添加深色系的效果；
    - `link-body-emphasis`：该样式类会为其作用的链接文本元素添加强调色系的效果；
  - 如果我们想设置链接文本样式中的文本不透明度，可以考虑使用`link·opacity-*`这组样式类，其中`*`的取值及其具体的效果如下：
  ...
  - 如果我们还想设置鼠标悬停在链接之上时的文本透明度，也可以使用`link-opacity-*-hover`样式类，这里`*`的取值与`link-opacity-*`相同，效果当然也是相同的，这里就不再复述了。
  - 如果我们想设置的是链接文本样式中下划线的颜色，可以考虑使用`link-underline-*`这组样式类来进行设置，这里`*`的取值与`link-*`相同，效果当然也是相同的，这里就不再复述了。
  - 如果我们还想设置鼠标悬停在链接之上时下划线的颜色，可以考虑使用`link-underline-*-hover`这组样式类来进行设置，这里`*`的取值与`link-*`相同，效果当然也是相同的，这里就不再复述了。
  ...
  - 如果我们想设置的是链接文本样式中下划线与文本之间的距离，可以考虑使用`link-offset-*`这组样式类来进行设置，其中`*`的取值及其具体的效果如下：
    - `link-offset-1`：该样式类会为其作用的链接文本元素设置`1em`的下划线与文本之间的距离；
    - `link-offset-2`：该样式类会为其作用的链接文本元素设置`2em`的下划线与文本之间的距离；
    - `link-offset-3`：该样式类会为其作用的链接文本元素设置`3em`的下划线与文本之间的距离；
  - 如果我们还想设置鼠标悬停在链接之上时下划线与文本之间的距离，可以考虑使用`link-offset-*-hover`这组样式类来进行设置，其中`*`的`link-offset-*`相同，效果当然也是相同的，这里就不再复述了。
...
- **列表元素样式**：对于页面中用`<ul>+<li>`、`<ol>+<li>`这两组标记定义的无序列表或有序列表元素，Bootstrap框架中提供了`list-*`这组样式类来辅助设计师们定义它们的样式，其中`*`的取值及其具体的效果如下：
...
- **表格元素样式**：在使用Bootstrap框架设计表格元素的样式时，`table`基本上是必须要用到的样式类,它会为表格设置一些基本的样式。在这个基本样式的基础上，Bootstrap框架中又提供了`table-*`这组样式类来辅助设计师们设置一些更复杂的表格样式，其中`*`的取值及其具体的效果如下：
  - `table-striped`：该样式类会为表格类元素中的奇数行设置为浅灰色背景，偶数行设置为白色背景；
  - `table-bordered`：该样式类会为表格类元素中的所有单元格设置边框；
  - `table-borderless`：该样式类会为表格类元素中的所有单元格去除边框；
  - `table-hover`：该样式类会为表格类元素中的所有单元格设置鼠标悬停效果；
  - `table-condensed`：该样式类会为表格类元素中的所有单元格设置紧凑的间距；
  - `table-responsive`：该样式类会为表格类元素设置响应式表格效果，使得表格类元素在移动设备上也可以正常显示；
  而对于表格中用`<thead>`标记定义的表头区域，Bootstrap框架中还提供了`thead-*`这组样式类，其中`*`的取值及其具体的效果如下：
    - `thead-inverse`：该样式类会为表格类元素中的表头单元格设置为浅灰色背景；
  对于表格中用`<tr>`标记定义的行元素，Bootstrap框架中还提供了`row-*`这组样式类，其中`*`的取值及其具体的效果如下：
    - `row-inverse`：该样式类会为表格类元素中的行元素设置为浅灰色背景；
  对于表格中用`<th>`标记定义的表头单元格，Bootstrap框架中还提供了`thead-*`这组样式类，其中`*`的取值及其具体的效果如下：
    - `thead-inverse`：该样式类会为表格类元素中的表头单元格设置为浅灰色背景；

- **图片元素样式**：对于页面中用`<img>`标记定义的图片元素，Bootstrap框架中提供了`img-*`这组样式类，其中`*`的取值及其具体的效果如下：
  - `img-rounded`：该样式类会为图片类元素设置圆角效果；
  - `img-circle`：该样式类会为图片类元素设置圆形效果；
  - `img-thumbnail`：该样式类会为图片类元素设置边框效果；
