...
- `is-invalid`：该样式类主要用于表示表单元素处于无效状态。当表单元素的值不符合验证规则时，可以添加这个类，使其显示为红色边框和红色文字。
- `was-validated`：该样式类主要用于表示表单元素已经通过验证。当表单元素的值符合验证规则时，可以添加这个类，使其显示为绿色边框和绿色文字。

此外，还有一些其他的样式类可以用于自定义表单验证的外观和行为，例如`invalid-feedback`用于显示验证错误的消息。需要注意的是，使用这些样式类需要配合相应的JavaScript或jQuery代码来处理表单验证逻辑。
