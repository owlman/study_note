...
- `bg-*`样式类用于设置网页整体或者特定页面元素的背景颜色，它通常会为整个网页奠定基本的色彩基调。通过使用不同的背景颜色，设计师们可以清晰地划分出网页的不同区域，并给用户带来视觉上的层次感。这组样式类可能采用的颜色具体如下：
  - `bg-primary`类：效果为蓝色或绿色的背景；
  - `bg-secondary`类：效果为灰色或浅色的背景；
  - `bg-success`类：效果为绿色的背景；
  - `bg-danger`类：效果为红色的背景；
  - `bg-warning`类：效果为橙色的背景；
  - `bg-info`类：效果为浅蓝色或浅绿色的背景；
  - `bg-light`类：通常用于设置浅色系的背景；
  - `bg-dark`类：通常用于设置暗色系的背景；
  - `bg-transparent`类，通常用于设置透明的背景。
- `text-*`样式类用于设置网页中的文本类元素颜色，它可以使得文本内容在网页中更加突出和易于阅读。设计师们通常根据文本的重要性来选择不同的文本颜色，例如主要文本、次要文本、成功信息、警告信息等。这组样式类可能采用的颜色具体如下：
  - `text-primary`类：采用与`bg-primary`相同的文本颜色，用于表示主要文本；
  - `text-secondary`类：采用与`bg-secondary`相同的文本颜色，用于表示次要文本；
  - `text-success`类：采用与`bg-success`相同的文本颜色，用于表示带有成功信息的文本；
  - `text-danger`类：采用与`bg-danger`相同的文本颜色，用于表示带有危险信息的文本；
  - `text-warning`类：采用与`bg-warning`相同的文本颜色，用于表示带有警告信息的文本；
  - `text-info`类：采用与`bg-info`相同的文本颜色，用于表示一些具有提示意义的文本；
  - `text-muted`类: 采用与`bg-dark`相同的文本颜色，用于表示一些具有静默意义的文本；
  - `text-light`类：采用与`bg-light`相同的文本颜色，用于设置亮色系的文本颜色；
  - `text-dark`类：采用与`bg-dark`相同的文本颜色，用于设置暗色系的文本颜色；
...
当然了，我们在这里所示范的只是使用Bootstrap框架进行网页整体设计的初体验，目的是让读者对该框架能有一个大致的了解。接下来，我们还将结合网页设计工作中的其他任务做更多更为复杂的布局演示，

## 图文信息排版

在完成了网页的整体设计工作之后，设计师们接下来的工作就是安排要如何显示网页中的具体内容了。而在网页可显示的诸多元素中，最基本的就是图文类元素了，这类元素主要包括标题、段落、引用、列表、表格、链接、图片等。下面，就让我们来继续介绍一下Bootstrap框架中可用于图文信息排版的样式类和界面组件。
