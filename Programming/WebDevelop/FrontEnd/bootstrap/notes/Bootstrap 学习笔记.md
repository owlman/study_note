...
在完成了网页的整体设计工作之后，设计师们接下来的工作就是安排要如何显示网页中的具体内容了。而在网页可显示的诸多元素中，最基本的就是图文类元素了，这类元素主要包括标题、段落、强调、引用、链接、列表、表格、图片等。下面，让我们来继续介绍Bootstrap框架中可用于图文信息排版的样式类和界面组件。和之前一样，我们会先通过设计一个简单的示例来演示一下这些样式类和组件在图文排版任务中的应用，以便读者能对这一工作任务有一个初步的了解，该示例的构建步骤如下：
