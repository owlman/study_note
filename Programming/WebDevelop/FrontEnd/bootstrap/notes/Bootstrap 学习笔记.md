...
除了纯文本类的元素，设计师在网页设计工作中还经常会用到列表、表格、图片等图表类元素，以便用于展示一些特定内容的信息。Bootstrap框架中为这些图表类元素提供了多种样式类，我们只需要根据需要，在相应的元素上应用对应的样式类即可。下面，让我们继续介绍这部分的样式类。
...
在完成了表单的整体布局之后，接下来就是设置表单中的交互元素了。在Bootstrap框架中，用于设置表单中交互元素的样式类主要有如下几个：
...
此外，Bootstrap还提供了一些JavaScript组件，如表单验证插件，可以通过JavaScript或jQuery代码来处理表单验证逻辑。这些组件可以帮助你在用户输入不符合预期时显示错误消息，并在表单提交时进行验证。
...
- `-sm`：表示样式类适用于小屏幕设备（屏幕宽度小于768像素）。
- `-md`：表示样式类适用于中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）。
- `-lg`：表示样式类适用于大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）。
- `-xl`：表示样式类适用于超大屏幕设备（屏幕宽度大于等于1200像素）。
...
- `d-sm-block`：该样式类用于设置元素在小于768像素的屏幕尺寸下显示；
- `d-md-block`：该样式类用于设置元素在768像素至992像素的屏幕尺寸下显示；
- `d-lg-block`：该样式类用于设置元素在992像素至1200像素的屏幕尺寸下显示；
- `d-xl-block`：该样式类用于设置元素在大于等于1200像素的屏幕尺寸下显示；
- `d-none`：该样式类用于设置元素在特定屏幕尺寸下隐藏；
- `hidden-sm`：该样式类用于设置元素在小屏幕设备（屏幕宽度小于768像素）下隐藏；
- `hidden-md`：该样式类用于设置元素在中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）下隐藏；
- `hidden-lg`：该样式类用于设置元素在大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）下隐藏；
- `hidden-xl`：该样式类用于设置元素在大屏幕设备（屏幕宽度大于等于1200像素）下隐藏；

- **栅格布局系统**：在Bootstrap框架中，我们可以通过以下扩展类对栅格布局方式进行补充，使其能更好地适应不同的屏幕尺寸：
  - `col-sm-*`：该扩展类用于设置在小屏幕设备（屏幕宽度小于768像素）下，元素所占的列数；
  - `col-md-*`：该扩展类用于设置在中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）下，元素所占的列数；
  - `col-lg-*`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）下，元素所占的列数；
  - `col-xl-*`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于1200像素）下，元素所占的列数；
  除此之外，Bootstrap框架还为它的网格系统提供了以下基于响应式设计的扩展：
  - `offset-sm-*`：该扩展类用于设置在小屏幕设备（屏幕宽度小于768像素）下，元素所偏移的列数；
  - `offset-md-*`：该扩展类用于设置在中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）下，元素所偏移的列数；
  - `offset-lg-*`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）下，元素所偏移的列数；
  - `offset-xl-*`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于1200像素）下，元素所偏移的列数；
- `order-sm-*`：该扩展类用于设置在小屏幕设备（屏幕宽度小于768像素）下，元素的排序顺序；
- `order-md-*`：该扩展类用于设置在中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）下，元素的排序顺序；
- `order-lg-*`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）下，元素的排序顺序；
- `order-xl-*`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于1200像素）下，元素的排序顺序；

- **表单交互元素**：针对表单中交互元素的响应式设计，Bootstrap框架也提供了以下扩展类：
  - `form-control-sm`：该扩展类用于设置在小屏幕设备（屏幕宽度小于768像素）下，表单控件的样式；
  - `form-control-md`：该扩展类用于设置在中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）下，表单控件的样式；
  - `form-control-lg`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）下，表单控件的样式；
  - `form-control-xl`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于1200像素）下，表单控件的样式；
  - `btn-block-sm`：该扩展类用于设置在小屏幕设备（屏幕宽度小于768像素）下，按钮的样式；
  - `btn-block-md`：该扩展类用于设置在中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）下，按钮的样式；
  - `btn-block-lg`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）下，按钮的样式；
  - `btn-block-xl`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于1200像素）下，按钮的样式；

- **导航栏组件**：针对导航栏组件的响应式设计，Bootstrap框架同样也提供了以下扩展类：
  - `navbar-expand-sm`：该扩展类用于设置在小屏幕设备（屏幕宽度小于768像素）下，导航条的样式；
  - `navbar-expand-md`：该扩展类用于设置在中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）下，导航条的样式；
  - `navbar-expand-lg`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）下，导航条的样式；
  - `navbar-expand-xl`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于1200像素）下，导航条的样式；

- **图文卡片组件**：针对图文卡片组件的响应式设计，Bootstrap框架同样也为其中的图片元素提供了以下扩展类：
  - `card-img-*-sm`：该扩展类用于设置在小屏幕设备（屏幕宽度小于768像素）下，卡片图片的样式；
  - `card-img-*-md`：该扩展类用于设置在中屏幕设备（屏幕宽度大于等于768像素，小于等于992像素）下，卡片图片的样式；
  - `card-img-*-lg`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于992像素，小于等于1200像素）下，卡片图片的样式；
  - `card-img-*-xl`：该扩展类用于设置在大屏幕设备（屏幕宽度大于等于1200像素）下，卡片图片的样式；
  在上述扩展类中，`*`的取值可以是`top`、`bottom`和`overlay`，分别用于表示目标图片是位于卡片的上部或下部，还是为卡片的背景。
