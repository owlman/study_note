...
3. **响应式栅格布局**：如果要采用这种布局样式，设计师需要使用`container`、`row`、`col-*`这三种样式类来组织页面的内容。具体来说就是，Bootstrap框架会将网页视为一个由多行多列组成的栅格系统。我们在设计网页时通常需要先用被设置了`row`类的布局类标记来定义好该网格系统的每一行（可设置的行数在理论上是不受限制的），然后对于栅格系统中每一行，我们通常会将其划分为12列，并使用`col-*`类来定义具体元素该栅格系统中所占的列数。下面是采用该布局系统的一个示意图：
