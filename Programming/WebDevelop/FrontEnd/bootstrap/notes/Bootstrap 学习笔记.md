...
3. **栅格系统布局**：如果要采用这种布局样式，设计师需要使用`container`、`row`、`col-*`这三种样式类来完成页面的布局。具体来说就是，Bootstrap框架会将网页视为一个由多行多列组成的栅格系统。我们在设计网页时通常需要先用被设置了`row`类的布局类标记来定义好该栅格系统的每一行（行数在理论上是不受限制的）。而栅格系统中的每一行通常可以被划分为12列，我们通常可以使用`col-*`类来定义页面元素在该栅格系统中所占的列数（其中的`*`可以取值从1到12的整数）。下面是使用该系统进行页面布局的一个示意图：
